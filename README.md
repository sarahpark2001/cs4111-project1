...
| staffs             | staff_id (integer), email (text), name (text), phone_number (text), pay_grade (text), component (text), job_title (text), password (text)                                  | Contains all staff members, used for `staff` login and on the `Directory` page. Entries can be added via `staff` sign-up and modified via edit feature.                                                                                                                                         |
| student_attends    | student_id (integer), email (text), name (text), program_option (text), total_points (integer, default 0), year (integer), school_name (text), password (text)             | Contains all students, used for `student` login and on the `Directory` page. `total_points` defaults to 0 upon signup, with unique `student_id` auto-assigned. Entries can be modified by `students` or `staff` through edit features; they can be deleted through the staff edit feature.                           |
